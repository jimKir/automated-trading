...
def metrics(returns: pd.Series, label: str = "") -> dict:
    ...
    if len(r) < 5:
        return {"label": label, "sharpe": np.nan, "sortino": np.nan,
                ...
                "vol": np.nan, "n_days": len(r), "win_rate": np.nan,
...
for r in fold_results:
    print(f"{r['label']:<28} {r['sharpe']:>7.3f} {r['sortino']:>8.3f} {r['cagr']:>7.2f} "
          f"{r['max_dd']:>8.2f} {r['calmar']:>8.3f} {r['vol']:>6.2f} {r['win_rate']:>6.1f} {r['n_days']:>5}")
print("-" * 90)
print(f"{full_strat['label']:<28} {full_strat['sharpe']:>7.3f} {full_strat['sortino']:>8.3f} {full_strat['cagr']:>7.2f} "
...
print("-" * 90)
print(f"{full_spy['label']:<28} {full_spy['sharpe']:>7.3f} {full_spy['sortino']:>8.3f} {full_spy['cagr']:>7.2f} "
...
if len(aligned) > 20:
    ...
    info_ratio = ((aligned["strat"] - aligned["spy"]).mean() * 252) / track_err if track_err > 0 else np.nan
...
n_folds_positive_alpha = sum(1 for f, s in zip(fold_results, fold_spy_results)
                              if f["sharpe"] > s["sharpe"])
...
strat_ret.to_frame("strategy").to_csv(Path(__file__).parent / "results" / "wf_12m_strat_returns.csv")
...
if len(common_idx) > 0:
    ax1.plot(common_idx, cum_strat.loc[common_idx], color=COLORS["strategy"], lw=2.5,
             label=f"Strategy (Sharpe={full_strat['sharpe']:.2f}, CAGR={full_strat['cagr']:.1f}%)")
    ax1.plot(common_idx, cum_spy.loc[common_idx], color=COLORS["spy"], lw=2, ls="--",
...
for i, (fold_name, (s, e)) in enumerate(WF_FOLDS.items()):
    ...
    ax1.text(mid, ax1.get_ylim()[1] if ax1.get_ylim()[1] != 1 else 1.02,
             fold_name.split("(")[1].rstrip(")"), fontsize=8, ha="center",
             va="bottom", color=COLORS["muted"])
...
ax1.set_title("Equity Curve — Strategy vs SPY", fontsize=12, fontweight="bold", color=COLORS["text"])
...
ax4.plot(roll_sharpe_s.index, roll_sharpe_s, color=COLORS["strategy"], lw=1.5, alpha=0.8, label="Strategy")
ax4.plot(roll_sharpe_b.index, roll_sharpe_b, color=COLORS["spy"], lw=1.2, ls="--", alpha=0.7, label="SPY")
...
ax4.set_title("Rolling Sharpe Ratio (21-Day Window)", fontsize=12, fontweight="bold", color=COLORS["text"])
...
plt.savefig(str(results_path.parent / "wf_12m_oos_chart.png"), dpi=150,
