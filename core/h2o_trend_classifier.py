...
def _parkinson_vol(high: pd.Series, low: pd.Series, window: int = 10) -> pd.Series:
    ...
    return (
        ...
        .rolling(window)
        .mean()
        .apply(lambda x: np.sqrt(x / (4 * np.log(2)) * 252) if x > 0 else 0)
    )
...
class H2OTrendClassifier:
    ...
    def _try_load_h2o(self):
        ...
        try:
            ...
            if self._h2o_model is None:
                log.info(
                    "TrendClassifier: H2O available, no pre-trained model — will train on first call"
                )
    ...
    def build_features(
        ...
        if high is not None and low is not None:
            ...
            feats["price_pos_63"] = (close - low.rolling(63).min()) / (
                high.rolling(63).max() - low.rolling(63).min()
            ).replace(0, np.nan)
        ...
        if volume is not None and (volume > 0).sum() > 50:
            feats["vol_ratio_5_20"] = volume.rolling(5).mean() / volume.rolling(20).mean().replace(
                0, np.nan
            )
            feats["vol_ratio_10_50"] = volume.rolling(10).mean() / volume.rolling(
                50
    ...
    def _build_labels(self, close: pd.Series, horizon: int = 10) -> pd.Series:
        ...
        labels = fwd_ret.rank(pct=True).apply(
            lambda x: (
                0 if x < 0.20 else (1 if x < 0.40 else (2 if x < 0.60 else (3 if x < 0.80 else 4)))
            )
        )
        return labels
    ...
    def train(
        ...
        log.info(
            f"TrendClassifier: training on {len(train_df)} rows, "
        ...
        try:
            ...
            log.info(f"TrendClassifier: trained successfully. Logloss={perf.logloss():.4f}")
            return True
