...
class PriceVolumeSegmenter:
    ...
    def extract_features(
        ...
        if as_of_date is not None:
            ...
            if open_price is not None: open_price = open_price[open_price.index <= as_of_date]
        ...
        if volume is not None and (volume > 0).sum() > 25:
            ...
            feats['vol_pct_chg_5d'] = float(
                (volume.iloc[-5:].mean() / vol_20d_avg.iloc[-1] - 1)
            ) if vol_20d_avg.iloc[-1] > 0 else 0.0

            feats['vol_pct_chg_10d'] = float(
                (volume.iloc[-10:].mean() / vol_20d_avg.iloc[-1] - 1)
            ) if vol_20d_avg.iloc[-1] > 0 else 0.0
            ...
            if open_price is not None and len(open_price) >= 5:
                ...
                vol_5 = volume.iloc[-5:]
                total_vol = vol_5.sum()
                feats['buying_pressure_5d'] = float(
                    vol_5[up_days].sum() / total_vol
