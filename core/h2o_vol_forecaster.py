...
_DEFAULT_SYMBOLS = [
    'SPY','GLD','SLV','TLT','HYG','SHY',
    'EEM','EWZ','EWY','EWC','EWJ','EWU','VGK',
    'SOXX','IBB','XBI','COPX','BTC-USD'
]
...
class H2OVolForecaster:
    ...
    @classmethod
    def load(cls) -> Optional['H2OVolForecaster']:
        """
    ...
    def _build_features(
        ...
        if len(ret_arr) >= 30:
            r_slice = ret_arr[-25:]
            rvols   = [np.sqrt(np.mean(r_slice[max(0,i-5):i]**2)*252)
...
def retrain(train_end: str = None):
    ...
    if not train_script.exists():
        ...
        return
    log.info(f'Retraining H2O vol model...')
...
if __name__ == '__main__':
    ...
    if '--retrain' in sys.argv:
        ...
        if fc is None:
            ...
            spy = yf.download('SPY', start='2020-01-01', end='2026-03-21',
                              auto_adjust=True, progress=False)
            r = np.log(spy['Close'] / spy['Close'].shift(1)).dropna().squeeze()
            vix = yf.download('^VIX', start='2020-01-01', end='2026-03-21',
