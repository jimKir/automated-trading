...
class AlpacaDataFetcher:
    ...
    def _cache_load(self, path: Path, ttl_hours: int = 4) -> Optional[dict]:
        if not path.exists():
            return None
        try:
    ...
    def _cache_save(self, path: Path, value):
        try:
            path.write_text(json.dumps({"v": value, "_ts": time.time()},
                                        default=str))
        except Exception:
            pass
    ...
    def get_option_chain(
        ...
        try:
            ...
            for cid, snap in chain.items():
                ...
                delta = None
                if getattr(snap, "greeks", None) and snap.greeks and getattr(snap.greeks, "delta", None) is not None:
                ...
                elif getattr(snap, "latest_trade", None) and snap.latest_trade and getattr(snap.latest_trade, "size", None):
                ...
                if getattr(snap, "latest_quote", None) and snap.latest_quote:
                    bid = float(snap.latest_quote.bid_price) if getattr(snap.latest_quote, "bid_price", None) else 0.0
                    ask = float(snap.latest_quote.ask_price) if getattr(snap.latest_quote, "ask_price", None) else 0.0
...
class VWAPDistanceSignal:
    ...
    def compute(
        ...
        """
        start = datetime(as_of_date.year, as_of_date.month, as_of_date.day) \
                - timedelta(days=self.lookback_days + 30)
    ...
    def compute(
        ...
        for sym in symbols:
            try:
                ...
                z = (fill_rate - fill_rate.rolling(63).mean()) / \
                    fill_rate.rolling(63).std().replace(0, np.nan)
...
class TradeIntensitySignal:
    ...
    def compute(
        ...
        """Returns {symbol: signal} in [-1, +1]."""
        start = datetime(as_of_date.year, as_of_date.month, as_of_date.day) \
                - timedelta(days=self.baseline_days + 30)
        end   = datetime(as_of_date.year, as_of_date.month, as_of_date.day)
        ...
        for sym in symbols:
            try:
                ...
                size_z    = (avg_size - avg_size.rolling(self.baseline_days).mean()) / \
                            avg_size.rolling(self.baseline_days).std().replace(0, np.nan)
                intensity = (size_z * ret_sign).rolling(self.lookback_days).mean()
                # Final z-score
                z = (intensity - intensity.rolling(63).mean()) / \
                    intensity.rolling(63).std().replace(0, np.nan)
                score = float(z.dropna().iloc[-1]) if not z.dropna().empty else 0.0
                raw_scores[sym] = score
            except Exception as e:
...
class AlpacaMicrostructureSignal:
    ...
    def __init__(self, config: dict = None, api_key: str = "", api_secret: str = ""):
        ...
        log.info(f"AlpacaMicrostructure: enabled={self.enabled} "
