...
parser.add_argument(
    "--download-missing", action="store_true", help="Auto-download missing symbols via yfinance"
)
...
print(f"\n{'Symbol':<12} {'Status':<12} {'Rows':>7} {'From':<12} {'To':<12} {'Days Stale':>11}")
print("\u2500" * 68)
...
for symbol in ALL_SYMBOLS:
    ...
    if df is None or len(df) == 0:
        ...
        if days_old > 5:
            stale.append(symbol)
        print(
            f"{symbol:<12} {status:<12} {len(df):>7} "
...
print(f"\n{line_sep}")
print(
    f"Total: {len(ALL_SYMBOLS)} | OK: {len(ALL_SYMBOLS) - len(missing) - len(stale)} "
...
if missing and args.download_missing:
    ...
    for sym in missing:
        ...
        if not df.empty:
            ...
            fname = (
                sym.replace("/", "_").replace("-", "_").replace("=", "_").replace("^", "")
                + ".parquet"
            )
