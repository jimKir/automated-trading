...
C = {
    "strat":   "#20808D", "spy":   "#A84B2F", "btc":  "#944454",
    "bg":      "#F7F6F2", "surface":"#F9F8F5", "border":"#D4D1CA",
    "text":    "#28251D", "muted": "#7A7974",  "grid": "#E8E6E0",
    "green":   "#437A22", "red":   "#A12C7B",  "amber":"#964219",
}
...
class PaperTradingEmulator:
    ...
    def _get_regime(self, vix_series: pd.Series, spy_close: pd.Series,
    ...
    def run(self, all_data: dict[str, pd.DataFrame]) -> dict:
        ...
        price_df_full.index = pd.to_datetime(price_df_full.index).tz_localize(None) \
            if price_df_full.index.tz is not None else price_df_full.index
        ...
        for date in paper_dates:
            ...
            for sym, pos_val in self.positions.items():
                ...
                try:
                    ...
                    price_prev  = float(df["Close"].asof(
            ...
            log.info(f"\n{'─'*55}")
            log.info(f"REBALANCE {date.date()}  regime={regime}  "
            ...
            try:
                ...
                price_close_df = pd.DataFrame({
                    sym: df["Close"] for sym, df in hist.items()
                    if "Close" in df.columns
                })
                pos_scales = self.pos_scorer.score_today(price_close_df,
            ...
            if not active:
                ...
                for sym in list(self.positions.keys()):
                    ...
                    self.cash += val - cost
                    self.trade_log.append({"date": date, "sym": sym,
                                           "action":"sell","value":val,"cost":cost})
                continue
            ...
            # Log signal details
            log.info(f"  Signals: {len(longs)} longs  "
                     f"EWS={ews_colour}({ews_scale:.0%})  "
                     f"target_heat={heat:.0%}")
            crypto_scales_log = {s: round(pos_scales.get(s,1.0),2)
            ...
            for sym in list(self.positions.keys()):
                if sym not in target_values:
                    ...
                    self.trade_log.append({
                        "date": date, "sym": sym, "action": "close",
                        "value": round(val,2), "cost": round(cost,4),
                        "regime": regime,
                    })
                ...
                if delta > 0:
                    if self.cash >= delta + cost:
                        ...
                        self.trade_log.append({
                            ...
                            "value": round(delta,2), "cost": round(cost,4),
                            "regime": regime,
                        })
                else:
                    reduce = abs(delta)
            ...
            log.info(f"  Portfolio: ${self.equity:,.0f}  cash={self.cash/self.equity:.0%}"
...
def generate_chart(results: dict) -> str:
    ...
    # ── Equity curve ─────────────────────────────────────────────────────────
    ax_eq.plot(strat_cum.index, strat_cum.values, color=C["strat"],
               lw=2.2, label="Strategy", zorder=5)
    ax_eq.plot(spy_cum.index, spy_cum.values, color=C["spy"],
               lw=1.5, linestyle="--", alpha=0.85, label="SPY", zorder=4)
    ...
    ax_eq.legend(loc="upper left", framealpha=0.92, facecolor="white",
                 edgecolor=C["border"], fontsize=10, labelcolor=C["text"])
    ...
    # ── Drawdown ─────────────────────────────────────────────────────────────
    ax_dd.fill_between(dd.index, dd.values*100, 0,
                       alpha=0.5, color=C["strat"], label="Strategy DD", zorder=5)
    ax_dd.plot(spy_dd.index, spy_dd.values*100, color=C["spy"],
               lw=1.2, linestyle="--", alpha=0.8, label="SPY DD", zorder=4)
    ...
    ax_dd.legend(loc="lower left", framealpha=0.9, facecolor="white",
                 edgecolor=C["border"], fontsize=8.5, labelcolor=C["text"])
    ...
    for i in range(len(bull)-1):
        if not bull.iloc[i]:
            ax_reg.axvspan(bull.index[i], bull.index[i+1], alpha=0.12,
                           color=C["red"], zorder=0)
    ax_reg.set_ylabel("VIX", color=C["muted"], fontsize=9)
    ax_reg.set_title("VIX + Regime (pink = bear)", fontsize=9,
                     color=C["text"], fontweight="bold", pad=4)
    ax_reg.legend(fontsize=8, framealpha=0.9, facecolor="white",
                  edgecolor=C["border"], labelcolor=C["text"])

    # ── Choppy score ─────────────────────────────────────────────────────────
    ch_s = eq_log["choppy_sc"]
    ax_cs.fill_between(ch_s.index, ch_s.values, 0,
                       alpha=0.6, color=C["strat"], label="Choppy score")
    ...
    ax_cs.set_title("ChoppyRegimeDetector (EWS Layer F)", fontsize=9,
                    color=C["text"], fontweight="bold", pad=4)
    ...
    ax_pos.bar(pos_df.index, pos_df["n_positions"], color=C["strat"],
               alpha=0.7, label="# Positions")
    ax_pos2 = ax_pos.twinx()
    ax_pos2.plot(pos_df.index, pos_df["cash_pct"], color=C["amber"],
                 lw=1.5, linestyle="--", label="Cash %")
    ...
    ax_pos.set_title("Position Count & Cash Allocation (rebalance dates)", fontsize=9,
                     color=C["text"], fontweight="bold", pad=4)
    lines1, labels1 = ax_pos.get_legend_handles_labels()
    lines2, labels2 = ax_pos2.get_legend_handles_labels()
    ax_pos.legend(lines1+lines2, labels1+labels2, loc="upper right",
...
def main():
    ...
    # Symbols to load
    UNIVERSE = [
        "SPY","QQQ","IWM","GLD","TLT","VGK","EEM","XLK","XLE","XLF",
        "VNQ","AGG","EWJ","XLV","EMXC",
        "BTC","ETH","SOL",    # crypto (local names)
        "ES","NQ","GC","CL",  # futures (local names)
        "VIX","HYG","LQD","DXY","NFLX","MMM",
    ...
    pd.DataFrame(results["trade_log"]).to_csv(
        "results/paper_trading_trades.csv", index=False)
    pd.DataFrame(results["equity_log"]).to_csv(
        "results/paper_trading_equity_log.csv", index=False)

    pos_rows = []
    for snap in results["position_log"]:
        row = {k:v for k,v in snap.items() if k != "positions"}
        row["top_positions"] = str(sorted(snap["positions"].items(),
    ...
    summary = {k:v for k,v in results.items()
               if k not in ("equity_log","position_log","trade_log")}
