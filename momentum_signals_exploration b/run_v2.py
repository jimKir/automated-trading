...
if not API_KEY or not API_SECRET:
    raise ValueError("Missing credentials. Set environment variables:\n"
                     "  export APCA_API_KEY_ID=your_key\n"
                     "  export APCA_API_SECRET_KEY=your_secret")

# ── symbol universe ────────────────────────────────────────────────────────
BASE_UNIVERSE = [
    "AAPL","MSFT","NVDA","GOOGL","GOOG","META","AMZN","TSLA","AVGO","JPM",
    "V","WMT","MA","UNH","XOM","LLY","JNJ","PG","COST","HD","MRK","ABBV",
    "CVX","BAC","PEP","KO","TMO","ACN","MCD","ABT","CRM","CSCO","GE","DHR",
    "LIN","TXN","AMD","NKE","HON","SBUX","QCOM","UPS","INTC","LOW","BA",
    "CAT","GS","AMAT","BLK","SCHW","C","WFC","MS","MU","LRCX","PANW","NOW",
    "INTU","ISRG","REGN","GILD","BKNG","ABNB","TGT","SLB","MPC","PSX","OKE",
    "COP","EOG","XOM","VLO","DVN","UNH","MDT","BMY","PFE","CVS","JNJ",
    "PLD","AMT","CCI","DLR","EQIX","NEE","DUK","SO","NFLX","DIS","T",
    "MMM","ROK","RTX","LMT","NOC","GD","L3","TDG","PH","ETN",
    "BRK.B","AXP","USB","PNC","TRV","ALL","CB","MET","PRU","AFL",
]
...
def print_table(title: str, rows: list, colour: str):
    ...
    hdr = (f"{'Symbol':<7} {'Score':>7} {'Price':>8}  "
...
def build_dashboard(result: dict, outfile: str = "momentum_v2_dashboard.html"):
    ...
    REGIME_BG = {
        "TRENDING_UP":   "#1b5e20", "TRENDING_DOWN": "#b71c1c",
        "TRANSITIONING": "#e65100", "CHOPPY":        "#37474f",
        "HIGH_FEAR":     "#4a148c", "ELEVATED_VOL":  "#f57f17",
        "UNKNOWN":       "#263238",
    }
    ...
    if not signals.empty:
        ...
        for _, row in top30.iterrows():
            ...
            chart_colors.append("'rgba(76,175,80,0.8)'" if row["score"] > 0 else "'rgba(244,67,54,0.8)'")

    consensus_badges = "".join(
        f"<span class='badge'>{s}</span>" for s in consensus
...
def main():
    ...
    print(f"\n{'='*72}")
    print(f"  MOMENTUM SCANNER V2  —  starting")
    print(f"{'='*72}\n")
