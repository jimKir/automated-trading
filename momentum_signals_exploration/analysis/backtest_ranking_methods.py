...
class RankingMethodsBacktest:
    ...
    def run_backtest(self, symbols: list[str], period: str = "1h", lookback_days: int = 30) -> dict:
        ...
        for method_name in ["standard", "volume_weighted", "surprise_factor"]:
            ...
            method_results = self._backtest_method(
                symbols=symbols, method_name=method_name, period=period, lookback_days=lookback_days
            )
    ...
    def _backtest_method(
        self, symbols: list[str], method_name: str, period: str, lookback_days: int
    ) -> dict:
        """Backtest single ranking method."""
        method_results = {"name": method_name, "signals": [], "metrics": {}}
        ...
        if returns:
            ...
            print(
                f"  ✅ {method_name}: Win rate {method_results['metrics']['win_rate']:.1%}, Avg return {method_results['metrics']['avg_return']:.3%}"
            )

        return method_results

    def _simulate_signal_outcome(self, symbol: str, method_name: str) -> float | None:
    ...
    def generate_backtest_report(
        self, backtest: dict, output_file: str = "ranking_backtest_report.json"
    ) -> str:
