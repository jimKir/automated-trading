...
class DataBentoTester:
    ...
    def test_consistency(self, symbols: list[str] = None, num_symbols: int = 20) -> dict:
        ...
        print("🔍 Testing data consistency between sources...")
        ...
        for symbol in symbols:
            try:
                ...
                print(
                    f"  {symbol}: Price diff {price_diff * 100:.3f}%, Volume diff {volume_diff * 100:.3f}%"
                )
        ...
        if price_diffs:
            consistency_report["summary"] = {
                ...
                if sum(price_diffs) / len(price_diffs) < 0.01
                else "Check data freshness",
            }
    ...
    def test_latency(self, symbol_counts: list[int] = None) -> dict:
        ...
        print("⏱️  Testing latency across different symbol counts...")

        if symbol_counts is None:
            symbol_counts = [10, 50, 100, 500, 1000]

        latency_report = {"timestamp": datetime.now().isoformat(), "tests": []}
            ...
            latency_report["tests"].append(
                {
                    ...
                    "faster_source": "alpaca"
                    if (alpaca_time and databento_time and alpaca_time < databento_time)
                    else "databento",
                }
            )
    ...
    def test_universe_expansion(self, sizes: list[int] = None) -> dict:
        ...
        print("📊 Testing universe expansion...")

        if sizes is None:
            sizes = [100, 500, 1000, 5000]

        expansion_report = {"timestamp": datetime.now().isoformat(), "tests": []}
        ...
        for size in sizes:
            ...
            try:
                ...
                expansion_report["tests"].append(
                    {
                        ...
                        "estimated_hourly_symbols": int(
                            3600 / max(elapsed / max(len(results), 1), 0.001)
                        ),
                    }
                )
                ...
                print(f"    Error: {str(e)[:50]}")
                expansion_report["tests"].append({"universe_size": size, "error": str(e)[:100]})
    ...
    def generate_integration_report(
        ...
        latency: dict,
        expansion: dict,
        output_file: str = "databento_integration_report.json",
    ) -> str:
...
def main():
    ...
    parser.add_argument(
        "--test-consistency", action="store_true", help="Compare Alpaca vs DataBento data"
    )
