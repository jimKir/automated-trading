...
from filters import RankingEngine, MomentumFilters
...
class RankingComparison:
    ...
    def compare_rankings(self, scan_results: List[Dict]) -> Dict:
        ...
        print(f"  Top symbol: {standard[0].get('symbol')} (momentum: {standard[0].get('combined_momentum', 0):.6f})")
        ...
        comparison['rankings']['volume_weighted'] = {
            ...
            'top_5': [
                {
                    'symbol': r.get('symbol'),
                    'weighted_momentum': round(r.get('combined_momentum', 0) * (r.get('volume', 1) / 1000000), 6),
                    'volume': r.get('volume', 0),
                    'return_pct': round(r.get('hourly_return_pct', 0), 3)
                }
        ...
        print(f"  Top symbol: {vol_weighted[0].get('symbol')} (volume: {vol_weighted[0].get('volume', 0):,.0f})")
        ...
        comparison['rankings']['surprise_factor'] = {
            ...
            'top_5': [
                {
                    'symbol': r.get('symbol'),
                    'surprise_score': round(r.get('combined_momentum', 0) * (r.get('volume', 1) ** 0.5) / 1000, 6),
    ...
    def generate_report(self, comparison: Dict, overlap: Dict, filtered_analysis: Dict,
                       output_file: str = 'ranking_comparison_report.json') -> str:
...
def main():
    ...
    parser.add_argument('--universe', default='sp500', help='Symbol universe (sp500, nasdaq100, all)')
    ...
    parser.add_argument('--min-magnitude', type=float, default=0.005, help='Minimum magnitude filter')
