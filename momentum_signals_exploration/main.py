...
def load_config(config_file: str = "config.json") -> dict:
    ...
    if config_path.exists():
        ...
        return {
            ...
            "alerts": {"console": True, "slack": False, "email": False},
        }
...
def run_scan(
    ...
    if filters_config:
        ...
        filtered_results = MomentumFilters().apply_all_filters(scanner.results, filters_config)

        # Re-rank filtered results
        gainers = sorted(
            filtered_results.items(), key=lambda x: x[1]["intra_momentum"], reverse=True
        )[:top_n]

        losers = sorted(filtered_results.items(), key=lambda x: x[1]["intra_momentum"])[:top_n]
...
def schedule_hourly_scan(
    universe: str = "sp500", filters_config: dict = None, alert_config: dict = None
) -> None:
    """
...
Examples:
  ...
  python main.py --universe sectors --action scan
    ...
    parser.add_argument(
        "--action", choices=["scan", "show-symbols"], default="scan", help="Action to perform"
    )

    parser.add_argument(
        "--schedule-hourly", action="store_true", help="Schedule scan to run every hour"
    )

    parser.add_argument(
        "--data-source", choices=["alpaca", "databento"], default="alpaca", help="Data source"
    )

    parser.add_argument(
        "--filters-config", default="config/filters.json", help="Path to filters config JSON"
    )

    parser.add_argument("--config", default="config.json", help="Path to main config JSON")

    parser.add_argument("--top-n", type=int, default=20, help="Number of top results to show")
    ...
    if Path(args.filters_config).exists():
        with open(args.filters_config) as f:
