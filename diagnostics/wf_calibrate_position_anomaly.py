...
def load_crypto(ticker: str, start: str = "2019-01-01") -> pd.Series:
    ...
    df = yf.download(ticker, start=start, end="2026-04-04",
...
def backtest_crypto(
    ...
    return {"sharpe": round(sh, 3), "max_dd": round(mdd * 100, 2),
            "cagr": round(ann * 100, 2), "n": n}
...
def run_wf_calibration(
    ...
    grid = list(itertools.product(
        G1_CEILINGS, G1_BASELINES, G3_DD_CEILS, SENSITIVITIES
    ))
    n_grid = len(grid)
    print(f"Grid size: {n_grid} combinations × {len(FOLDS)} folds × 2 crypto assets "
    ...
    for fold_label, is_s, is_e, oos_s, oos_e in FOLDS:
        ...
        print(f"  BTC  IS: Sh={best_is_res['BTC']['sharpe']:+.3f}  DD={best_is_res['BTC']['max_dd']:.1f}%")
        print(f"  BTC OOS: Sh={best_oos_res['BTC']['sharpe']:+.3f}  DD={best_oos_res['BTC']['max_dd']:.1f}%  "
              f"(base DD={base_btc['max_dd']:.1f}%  Δ={base_btc['max_dd']-best_oos_res['BTC']['max_dd']:+.1f}pp)")
        print(f"  ETH  IS: Sh={best_is_res['ETH']['sharpe']:+.3f}  DD={best_is_res['ETH']['max_dd']:.1f}%")
        print(f"  ETH OOS: Sh={best_oos_res['ETH']['sharpe']:+.3f}  DD={best_oos_res['ETH']['max_dd']:.1f}%  "
...
def derive_locked_params(fold_results: list[dict]) -> dict:
    ...
    locked["diagnostics"] = {
        ...
        "btc_mean_dd_reduction_pp": round(float(np.mean(btc_dd_reductions)), 2) if btc_dd_reductions else None,
        "eth_mean_dd_reduction_pp": round(float(np.mean(eth_dd_reductions)), 2) if eth_dd_reductions else None,
        "btc_is_sharpe_mean":  round(float(np.mean(btc_is_sharpes)), 3)  if btc_is_sharpes  else None,
        "btc_oos_sharpe_mean": round(float(np.mean(btc_oos_sharpes)), 3) if btc_oos_sharpes else None,
        "eth_is_sharpe_mean":  round(float(np.mean(eth_is_sharpes)), 3)  if eth_is_sharpes  else None,
        "eth_oos_sharpe_mean": round(float(np.mean(eth_oos_sharpes)), 3) if eth_oos_sharpes else None,
        "btc_is_oos_sharpe_gap": round(
            float(np.mean(btc_is_sharpes)) - float(np.mean(btc_oos_sharpes)), 3
        ) if btc_is_sharpes and btc_oos_sharpes else None,
    }
...
if __name__ == "__main__":
    ...
    for f in fold_results:
        p = f["params"]
        print(f"{f['fold']:<35} {p['g1_ceiling']:>6.2f} {p['g1_baseline']:>6.2f} "
    ...
    with open(RESULTS_FILE, "w") as f:
        json.dump({"folds": fold_results, "locked_params": locked},
