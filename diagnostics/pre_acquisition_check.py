...
_US_HOLIDAYS = np.array([
    ...
    "2023-11-24","2023-12-25","2024-01-01","2024-01-15","2024-02-19",
    "2024-03-29","2024-05-27","2024-06-19","2024-07-04","2024-09-02",
    "2024-11-28","2024-11-29","2024-12-25","2025-01-01","2025-01-09",
    "2025-01-20","2025-02-17","2025-04-18","2025-05-26","2025-06-19",
    "2025-07-04","2025-09-01","2025-11-27","2025-11-28","2025-12-25",
...
def check_data_exists(client, dataset: str, schema: str,
...
def check_ic_proxy(dataset: str, schema: str,
                   start: str, end: str) -> dict:
    """
...
def estimate_cost(dataset: str, schema: str,
                  start: str, end: str, n_syms: int) -> dict:
    days = trading_days(start, end)
    n_days = len(days)
...
def main():
    ...
    parser.add_argument("--symbols", type=int, default=20,
                        help="Number of symbols you plan to fetch (default: 20)")
    ...
    try:
        ...
        print(f"Databento client error: {e}"); sys.exit(1)
    ...
    cost = estimate_cost(args.dataset, args.schema,
                         args.start, args.end, args.symbols)
    print(f"\n{'─'*68}")
    print(f"  COST ESTIMATE (before touching API)")
    print(f"{'─'*68}")
    ...
    if cost['est_cost_usd'] > 100:
        print(f"  ⚠️  HIGH COST — validate signal IC before committing")
    elif cost['est_cost_usd'] > 20:
        print(f"  ⚠️  MODERATE COST — run IC proxy check first")
    else:
        print(f"  ✅ LOW COST — reasonable to proceed")

    # ── IC proxy check (free) ─────────────────────────────────────────────────
    print(f"\n{'─'*68}")
    print(f"  IC PROXY CHECK (free — yfinance price data)")
    print(f"  Measures predictive power of a cheap proxy for this schema")
    print(f"{'─'*68}")
    ...
    if ic_result.get("error"):
        ...
        if abs(ic) < 0.005 and p > 0.10:
            print()
            print(f"  ❌ WEAK PROXY SIGNAL — the underlying data is unlikely")
            print(f"     to have meaningful IC. Consider skipping this dataset.")
        elif abs(ic) >= 0.01 and p < 0.05:
            print()
            print(f"  ✅ STRONG PROXY SIGNAL — real data likely to have IC.")
            print(f"     Worth acquiring if cost is reasonable.")
        else:
            print()
            print(f"  ⚠️  MARGINAL PROXY SIGNAL — real data may or may not work.")
            print(f"     Consider a small-sample spot-fetch before full acquisition.")

    # ── Data existence check (free) ───────────────────────────────────────────
    print(f"\n{'─'*68}")
    print(f"  DATA EXISTENCE CHECK (free — limit=1 probes, 8 dates sampled)")
    print(f"{'─'*68}")
    exist = check_data_exists(client, args.dataset, args.schema,
                              args.start, args.end)
    if exist.get("error"):
        ...
        print(f"  Coverage:    {exist['hits']}/{exist['sample_size']} sampled dates "
              f"[{bar}] {hit_pct:.0f}%")
        if exist['first_good']:
    ...
    if not ic_result.get("error"):
        if abs(ic_result.get("ic", 0)) >= 0.01 and ic_result.get("p", 1) < 0.05:
            score += 1
            reasons.append(f"✅ Proxy IC significant ({ic_result['ic']:+.4f}, p={ic_result['p']:.3f})")
    ...
    if not exist.get("error"):
        if exist['hit_rate'] >= 0.75 and not exist['cutoff_warning']:
            score += 1
            reasons.append(f"✅ Data available across full range ({exist['hit_rate']*100:.0f}% coverage)")
        elif exist['hit_rate'] >= 0.5:
            score += 0.5
            reasons.append(f"⚠️  Partial data coverage ({exist['hit_rate']*100:.0f}%) or possible cutoff")
        else:
