...
try:
    ...
    print(
        f"  Cache: {_hc['real_files']} real files  |  "
...
for s in SYMS:
    try:
        df = yf.download(s, start="2022-06-01", end=OOS_END, auto_adjust=True, progress=False)
        if not df.empty:
            prices[s] = df
    except Exception:
        pass
...
def print_results(name, sig_df, wr_df, ics):
    ...
    print(f"\n  ── {name} ──")
    print(
        f"  Full Sharpe: {sharpe(r):+.3f}  Calm: {sharpe(r[cm]):+.3f}  Crisis: {sharpe(r[cr]):+.3f}"
    )
    print(
        f"  IC@5d:  {ic5[0]:+.4f}  p={ic5[1]:.4f}  {'✅' if ic5[1] < 0.05 else '⚠️' if ic5[1] < 0.10 else '❌'}"
    )
    print(
        f"  IC@10d: {ic10[0]:+.4f}  p={ic10[1]:.4f}  {'✅' if ic10[1] < 0.05 else '⚠️' if ic10[1] < 0.10 else '❌'}"
    )
    print(
        f"  IC@21d: {ic21[0]:+.4f}  p={ic21[1]:.4f}  {'✅' if ic21[1] < 0.05 else '⚠️' if ic21[1] < 0.10 else '❌'}"
    )
    print(
        f"  Permutation: p={p_val:.4f} ({z_sc:.1f}σ)  "
...
week_dates = [d.date() for d in wc.index if pd.Timestamp(OOS_START) <= d <= pd.Timestamp(OOS_END)]
...
_real_cached = sum(
    1 for f in _all_cache_files if f.stat().st_size > 100
)  # >100 bytes = has real data
print(f"  Cache dir : {_IMB_CACHE_DIR}")
print(f"  Cached files: {len(_all_cache_files)} total, {_real_cached} with real data")
print("  (Each weekly date uses 10 trading day fetches internally)")
...
try:
    ...
    for _cd in [
        d
        for d in step_dates
        if (d if isinstance(d, date) else date.fromisoformat(str(d))) not in _plan["missing"]
    ]:
        try:
            sigs = imb_signal.compute_weekly(SYMS, _cd)
            if sigs:
                imb_rows[pd.Timestamp(_cd)] = sigs
        except Exception:
            pass
...
for name, r in results.items():
    verdict = (
        "✅ USE"
        if r["perm_p"] < 0.05 and r["wf_pos"] >= 3
        else ("⚠️  WEAK" if r["perm_p"] < 0.10 or r["wf_pos"] >= 2 else "❌ SKIP")
    )
