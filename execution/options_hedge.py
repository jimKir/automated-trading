...
def estimate_put_premium(
    spy_price: float, strike_pct_otm: float, dte: int, vix_level: float, risk_free: float = 0.045
) -> float:
    """
...
class ProtectivePutHedge:
    ...
    def on_regime_change(
        ...
        log.info(
            f"[Hedge] Score={new_score:.3f} Regime={new_regime} "
        ...
        for pos in list(self.state.active_puts):
            if pos.is_expiring_soon:
                ...
                if new_regime in ("ORANGE", "RED"):
                    self._open_put(new_regime, spy_price, portfolio_equity, vix_level, today)

        # 3. Open new put if transitioning into ORANGE/RED with no position
        if (
            new_regime in ("ORANGE", "RED")
            and not self.state.active_puts
            and new_regime != self._last_regime
        ):
            put = self._open_put(new_regime, spy_price, portfolio_equity, vix_level, today)
    ...
    def summary(self) -> dict:
        return {
            ...
            "net_hedge_cost": round(
                self.state.total_premium_paid - self.state.total_payoff_received, 2
            ),
    ...
    def _open_put(
        self, regime: str, spy_price: float, portfolio_equity: float, vix_level: float, today: date
    ) -> PutPosition:
        ...
        log.info(
            f"[Hedge] OPENED {symbol} ×{contracts} "
    ...
    def _close_put(self, pos: PutPosition, spy_price: float, today: date) -> float:
        ...
        log.info(
            f"[Hedge] CLOSED {pos.symbol} payoff=${payoff:.0f} "
