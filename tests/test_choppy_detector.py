...
def mock_data():
    ...
    spy_df = pd.DataFrame(
        {"close": spy, "volume": np.random.randint(50_000_000, 200_000_000, 30)}, index=dates
    )
...
class TestInitialization:
    ...
    def test_order_flow_detector_exists(self, detector):
        assert hasattr(detector, "order_flow_detector"), "OrderFlowAnomalyDetector not wired"
