...
PERIODS = [
    ("COVID Crash & Recovery",
     "2018-01-01", "2020-02-01", "2020-12-31",
     "crash_recovery",
    ...
    ("Bear-to-Bull Recovery",
     ...
     "recovery_bull",
     "SVB collapse Mar 2023, AI bull from May. "
     "Should start YELLOW, fade to GREEN by Q2."),

    ("AI Bull Run",
     "2021-01-01", "2024-01-01", "2024-12-31",
     "sustained_bull",
...
C = {
    ...
    "spy":    "#A84B2F", "green":   "#437A22",  "amber":  "#964219",
    "red":    "#A12C7B",
}
...
def detector_stress_metrics(
    ...
    """
    ch = eq_log["choppy_sc"].reindex(spy_close.loc[period_start:period_end].index,
    ...
    avg_ews_scale = float(eq_log.get("ews_scale", pd.Series(1.0)).mean()) \
        if "ews_scale" in eq_log.columns else None
...
def performance_metrics(eq_log: pd.DataFrame, spy_close: pd.Series,
...
def run_period(
    ...
    total_cost = float(trade_log["cost"].sum()) if "cost" in trade_log.columns and n_trades > 0 else 0.0
...
def generate_chart(period_results: list[dict], spy_close: pd.Series) -> str:
    ...
    outer = GridSpec(n + 1, 1, figure=fig,
    ...
    for row_i, (res, (_, _, period_color)) in enumerate(
        ...
        inner = GridSpec(1, 3, figure=fig,
                         subplot_spec=outer[row_i],
                         wspace=0.28,
                         width_ratios=[2.5, 0.7, 0.8])
        ...
        # ── Equity curve ──────────────────────────────────────────────────
        ax_eq.plot(strat_cum.index, strat_cum.values,
                   color=period_color, lw=2.0, label="Strategy", zorder=5)
        ax_eq.plot(spy_cum.index, spy_cum.values,
                   color=C["spy"], lw=1.3, linestyle="--", alpha=0.8,
                   label="SPY", zorder=4)
        ...
        ax_eq.legend(loc="upper left", fontsize=7.5, framealpha=0.9,
                     facecolor="white", edgecolor=C["border"], labelcolor=C["text"])
        ax_eq.set_title(res["label"], fontsize=10, color=C["text"],
        ...
        ch_vals  = ch_s.values
        ax_ch.fill_between(ch_dates, ch_vals, 0,
                           alpha=0.6, color=period_color)
        ...
        ax_ch.set_title("Choppy Score", fontsize=8.5, color=C["text"],
        ...
        for y, label in [(0.17, "YLW"), (0.27, "ORG"), (0.40, "RED")]:
            if float(ch_s.max()) > y:
                ax_ch.text(0.02, y + 0.005, label, transform=ax_ch.get_yaxis_transform(),
        ...
        for text, color, size, weight in metrics_lines:
            ax_met.text(0.06, y_pos, text, transform=ax_met.transAxes,
    ...
    bars1 = ax_sum.bar(x - w/2, sharpes, w, label="Strategy Sharpe",
                       color=PERIOD_COLORS, alpha=0.85, zorder=3)
    bars2 = ax_sum.bar(x + w/2, spy_sh, w, label="SPY Sharpe",
                       color=C["spy"], alpha=0.55, zorder=3)
    ax_sum.axhline(0, color=C["muted"], lw=0.8, alpha=0.6)
    ax_sum.axhline(0.5, color=PERIOD_COLORS[3], lw=1.0, linestyle=":",
                   alpha=0.5, label="0.5 target")
    ...
    ax_sum.legend(loc="upper right", framealpha=0.92, facecolor="white",
                  edgecolor=C["border"], fontsize=9, labelcolor=C["text"])
    ax_sum.set_title(
        "Sharpe Ratio per Regime — Strategy vs SPY Benchmark",
        fontsize=11, color=C["text"], fontweight="bold", pad=7
    )
...
def print_summary(period_results: list[dict]) -> dict:
    ...
    print(f"{'='*110}")
    print(f"{'Period':<28} {'Strat%':>7} {'Sh':>6} {'DD%':>7} | "
...
def main():
    ...
    for label, warmup, start, end, _rtype, desc in PERIODS:
        ...
        try:
            ...
            p = res["performance"]
            d = res["detector"]
            log.info(f"  DONE: return={p.get('total_return_pct',0):+.1f}%  "
    ...
    full_summary = {
        "periods": [{k: v for k, v in r.items() if k not in ("equity_log", "position_log")}
                    for r in period_results],
        "aggregate": summary,
    }
