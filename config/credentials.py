...
def get_alpaca_credentials() -> tuple[str, str]:
    ...
    if not api_key:
        raise EnvironmentError("ALPACA_API_KEY environment variable not set")
    if not secret_key:
        raise EnvironmentError("ALPACA_SECRET_KEY environment variable not set")
    return api_key, secret_key


def get_databento_key() -> Optional[str]:
