...
def get_composite_stress_live() -> dict:
    ...
    return {"fed_stress": fed, "inflation_stress": infl, "recession_stress": rec, "composite": comp}
...
def backfill_fed_stress(start_date="2022-01-01") -> pd.Series:
    ...
    for mkt in hold_markets:
        ...
        for endpoint in [
            ...
            try:
                mkt_open_ts = int(datetime.fromisoformat(open_t).timestamp())
                mkt_close_ts = (
                    int(datetime.fromisoformat(close_t).timestamp())
                    if close_t
                    else int(time.time())
                )
                ...
                for c in candles:
                    ...
                    close_p = float(price_data.get("close_dollars") or price_data.get("close") or 0)
                    if close_p > 1:
                        close_p /= 100
                    if close_p > 0:
...
def backfill_inflation_stress(start_date="2022-01-01") -> pd.Series:
    ...
    s = s.reindex(pd.date_range(s.index.min(), s.index.max(), freq="D")).ffill()
    log.info(
        f"  Inflation stress: {s.index.min().date()} → {s.index.max().date()}, mean={s.mean():.3f}"
    )
    return s
...
def run(save=False):
    ...
    for fold in FOLDS:
        try:
            ...
            results.append(r)
            print(
                f"  {fold['name']:<18} {base_m['sharpe']:>12.3f} {kalshi_m['sharpe']:>13.3f} "
