...
class PutBook:
    ...
    def open_put(self, entry_date, spy_price, vix, equity,
                 otm_pct=0.02, dte=21, hedge_ratio=0.20,
                 max_contracts=5, regime="ORANGE"):
        ...
        pos = {"entry_date": entry_date, "expiry": expiry,
               "strike": strike, "contracts": n,
               "premium": premium, "cost": cost, "regime": regime,
               "closed": False, "payoff": 0.0}
        self.positions.append(pos)
        self.total_premium += cost
        self.log.append(f"{entry_date}: OPEN {regime} put "
                        f"K=${strike:.0f} exp={expiry} ×{n} cost=${cost:.0f}")
        return pos
...
def simulate(df, use_hedge=True, initial_equity=100_000.0):
    ...
    for a in weights:
        ...
        rets[a] = col.pct_change().fillna(0) if col is not None \
    ...
    for i in range(len(df)):
        ...
        if use_hedge:
            ...
            if (regime in ("ORANGE", "RED") and
                    last_regime not in ("ORANGE", "RED") and
                    not book.has_open()):
                otm = 0.02 if regime == "ORANGE" else 0.05
                dte = 21   if regime == "ORANGE" else 14
                pos = book.open_put(today, spy_price, vix, equity,
                                    otm_pct=otm, dte=dte, regime=regime)
                hedge_cost_frac = pos["cost"] / equity

            # Roll if all puts have expired
            if (regime in ("ORANGE","RED") and
                    not book.has_open() and last_regime in ("ORANGE","RED")):
                otm = 0.02 if regime == "ORANGE" else 0.05
                dte = 21   if regime == "ORANGE" else 14
                pos = book.open_put(today, spy_price, vix, equity,
                                    otm_pct=otm, dte=dte, regime=regime)
...
def make_chart(period, unhedged, hedged, spy_bah, save_path):
    ...
    for df_r, color, lw, ls, label in [
        ...
        m   = metrics(df_r)
        ax.plot(cum.index, cum.values, color=color, lw=lw, ls=ls,
    ...
    for key, color, label in [
        ...
        if key in period:
            try:
                ax.axvline(pd.Timestamp(period[key]), color=color,
                           ls=":", lw=1.5, alpha=0.8)
                ax.text(pd.Timestamp(period[key]), ax.get_ylim()[0]*1.02,
                        label, color=color, fontsize=8, rotation=90,
                        va="bottom")
            except Exception:
                pass
...
def run(save=False):
    ...
    for period in PERIODS:
        ...
        spy_bah = pd.DataFrame({"equity": df["spy"] / df["spy"].iloc[0] * 100_000},
                                index=df.index)
        ...
        result = {
            "period": period["name"], "label": period["label"],
            "unhedged": um, "hedged": hm, "spy_bah": sm,
    ...
    verdict = ("ADOPT" if n_dd==len(all_results) and avg_dd>0.02
               else "CONDITIONAL" if n_dd>0
               else "REJECT")
