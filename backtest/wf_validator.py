...
def walk_forward_test(
    ...
    for train_start, train_end, test_start, test_end in folds:
        ...
        train_rets = returns[(returns.index >= train_start) & (returns.index <= train_end)].dropna()

        test_rets = returns[(returns.index >= test_start) & (returns.index <= test_end)].dropna()
...
def sensitivity_test(
    ...
    is_plateau = (
        sharpes.max() - sharpes.min() < 0.3  # range < 0.3 Sharpe units
        and (sharpes > baseline_sharpe).sum() >= 4  # at least 4 of 7 values beat baseline
    )
    log.info(
...
def run_full_validation(
    ...
    log.info("=" * 65)
    log.info(
        f"  Method 1 — Walk-Forward:       {'PASS ✓' if wf_pass else 'FAIL ✗'}  "
        f"({wf_pass_count}/{wf_total} folds improved)"
    )
    log.info(
        f"  Method 2 — Sensitivity:        {'PASS ✓' if sens_pass else 'FAIL ✗'}  "
        f"({beat_baseline}/{len(sens_df)} target vols beat baseline)"
    )
    log.info(
        f"  Method 3 — Permutation test:   {'PASS ✓' if perm_pass else 'FAIL ✗'}  "
