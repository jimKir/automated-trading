...
def compute_choppy_score(df: pd.DataFrame) -> pd.Series:
    ...
    score = (
        (0.30 * vol_spike + 0.25 * vix_stress + 0.25 * vix_spike + 0.20 * mom).fillna(0).clip(0, 1)
    )
...
def simulate(
    df: pd.DataFrame, use_hedge: bool = True, initial_equity: float = 100_000.0
) -> pd.DataFrame:
    """
...
def make_chart(period: dict, unhedged: pd.DataFrame, hedged: pd.DataFrame, save_path: str):
    ...
    ax.plot(
        ...
        lw=2,
        ls="--",
        label=f"Unhedged  (MaxDD={metrics(unhedged, '')['max_dd']:.1%})",
    )
    ax.plot(
        hd_cum.index,
        hd_cum.values,
        color="steelblue",
        lw=2.5,
        label=f"Hedged    (MaxDD={metrics(hedged, '')['max_dd']:.1%})",
    )
    ...
    for i in range(len(hedge_on) - 1):
        if hedge_on.iloc[i]:
            ax.axvspan(hedge_on.index[i], hedge_on.index[i + 1], alpha=0.12, color="green", lw=0)
    ...
    ax2.fill_between(score.index, score, alpha=0.4, color="salmon", label="Choppy score")
    ...
    ax3.bar(hedge_pnl.index, hedge_pnl * 100, color=colors, alpha=0.7, width=1)
...
def make_summary_chart(all_results: list, save_path: str = "results/options_hedge_summary.png"):
    ...
    fig.suptitle("Protective Put Hedge — Impact Summary", fontsize=13, fontweight="bold")
    ...
    ax = axes[2]
    bars = ax.bar(
        periods, net_cost, color=["green" if v < 0 else "red" for v in net_cost], alpha=0.8
    )
...
def run(save: bool = False):
    ...
    for period in PERIODS:
        ...
        for key in ["total_return", "sharpe", "max_dd", "worst_day", "calmar"]:
            ...
            flag = (
                "✅"
                if (key in ("sharpe", "total_return", "calmar") and d > 0)
                or (key in ("max_dd", "worst_day") and d > 0)
                else "⚠"
                if abs(d) < 0.005
                else "❌"
            )
        ...
        print(
            f"    Net cost:        ${total_paid - total_payoff:+,.0f} "
        ...
        if save:
            make_chart(
                period, unhedged, hedged, f"results/options_hedge_{period['name'].lower()}.png"
            )
    ...
    print(
        f"  MaxDD improved:   {n_dd_better}/{len(all_results)} periods | "
    ...
    verdict = (
        "ADOPT — hedge pays off significantly in stress periods"
        if n_dd_better == len(all_results)
        else "CONDITIONAL — hedge helps in some but not all periods"
        if n_dd_better > 0
