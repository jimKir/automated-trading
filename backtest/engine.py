    ...
    def run(
        ...
        if self.config.get("position_anomaly", {}).get("enabled", True):
            try:
                ...
                log.warning(f"PositionAnomalyScorer init failed: {_pa_err} — skipping")
                _pos_anomaly_scorer = None
        ...
        for i, date in enumerate(all_dates):
            ...
            if date in rebalance_dates:
                ...
                # ── Per-symbol position anomaly scaling ──────────────────
                # Asymmetric: crypto floor=10% (up to 90% cut), equity floor=40%.
                # Independent of EWS — no double-counting of regime features.
                _pos_scales: dict = {}
                if _pos_anomaly_scorer is not None:
                    try:
                        ...
                        log.debug(f"PositionAnomalyScorer.score_day failed: {_pa_err}")
