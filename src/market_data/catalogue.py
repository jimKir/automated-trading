...
def _date_str(d: str | date | datetime | None) -> str | None:
    if d is None:
        return None
...
def _entry_key(
    source: str, dataset: str, schema: str, symbols: list[str], start: str, end: str
) -> str:
...
class DataCatalogue:
    ...
    def update_end(self, key: str, new_end: str | date, rows: int = 0):
    ...
    def find(
        ...
        for entry in self._data.values():
            if source and entry.get("source") != source.lower():
                continue
            if dataset and entry.get("dataset") != dataset:
                continue
            if schema and entry.get("schema") != schema:
                continue
            if symbol and symbol not in entry.get("symbols", []):
                continue
            if date_s:
    ...
    def find_all(
        ...
        for entry in self._data.values():
            ...
            if symbol and symbol not in entry.get("symbols", []):
                continue
            if freq_norm and entry.get("frequency") != freq_norm:
                continue
    ...
    def has(
        ...
        return self.find(source=source, schema=schema, symbol=symbol, date=date) is not None

    def coverage(self, source: str, schema: str, symbol: str) -> tuple | None:
    ...
    def summary(self, source: str | None = None):
        ...
        print("=" * 80)
        print(
            f"  DATA CATALOGUE  ({len(entries)} entries)"
            + (f"  filter: source={source}" if source else "")
        )
        print("=" * 80)
        print(
            f"  {'Source':<12} {'Dataset':<18} {'Schema':<14} {'Freq':<10} "
        ...
        for src in sorted(by_source):
            for e in sorted(by_source[src], key=lambda x: (x["dataset"], x["schema"])):
                syms = e.get("symbols", [])
                sym_str = syms[0] if len(syms) == 1 else f"{len(syms)} syms"
                rows = e.get("rows", 0)
                rows_s = f"{rows:,}" if rows else "—"
                print(
                    f"  {e['source']:<12} {e['dataset']:<18} {e['schema']:<14} "
...
_catalogue: DataCatalogue | None = None
