...
class EventShockDetector:
    ...
    # ------------------------------------------------------------------
    def _vix_velocity_score(self, vix: pd.Series,
        ...
        if chg >= VIX_VEL_CRISIS:
            return 1.0
        elif chg >= VIX_VEL_THRESHOLD:
            return 0.3 + 0.7 * (chg - VIX_VEL_THRESHOLD) / (VIX_VEL_CRISIS - VIX_VEL_THRESHOLD)
        return 0.0

    def _vix_term_structure_score(self, vix: pd.Series, vix3m: pd.Series,
                                  date: pd.Timestamp) -> float:
        """
    ...
    def _breadth_score(self, breadth: pd.Series, date: pd.Timestamp) -> float:
        ...
        if v <= BREADTH_CRISIS:
            return 1.0
        elif v <= BREADTH_STRESS:
            return 0.3 + 0.7 * (BREADTH_STRESS - v) / (BREADTH_STRESS - BREADTH_CRISIS)
        return 0.0

    def _cross_asset_shock_score(self, spy: pd.Series, tlt: pd.Series,
                                 gld: pd.Series, date: pd.Timestamp) -> float:
        """
    ...
    # ------------------------------------------------------------------
    def compute_series(self, start: str, end: str,
                       all_prices: pd.DataFrame = None) -> pd.Series:
        """
